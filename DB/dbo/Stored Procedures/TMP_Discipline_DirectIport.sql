...
/* Tests
-----------------------------------------
exec [dbo].[TMP_Field_DirectIport]
*/
-- =============================================
CREATE PROCEDURE [dbo].[TMP_Discipline_DirectIport]
	AS
BEGIN
	SET NOCOUNT ON;
...
FOR SELECT [ID]
      ...
      ,[Description]
      ,[ID_Language]     
FROM [dbo].[TMP_Discipline_DirectImport] 
where IsAdded = 0;
...
BEGIN	
	...
	if exists (Select 1 from [dbo].[TMP_Discipline_DirectImport] 
	where @ID = ID and IsAdded = 1 and ID_Imported is not null)
	BEGIN
		set @ttID = (Select ID_Imported from [dbo].[TMP_Discipline_DirectImport] 
	where ID = @ID )
	...
	BEGIN
		...
		BEGIN		
			...
			insert into Discipline (IsActive,ID_Translation)
			select 1, @ID_Trans
		...
		ELSE
		BEGIN
			Select @ID_Trans = a.ID_Translation from Discipline a where a.ID = @ttID
	...
	update [TMP_Discipline_DirectImport]
	set IsAdded =1,
		ID_Imported = @ttID
	where @ID = ID
	IF(@@ERROR > 0)
